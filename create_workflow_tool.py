...
def remove_workflow(workflow_name):
    """Remove a workflow from orchestrate_workflows.json."""
    workflows = load_workflows()

    if workflow_name not in workflows.get("workflows", {}):
        return {"status": "error", "message": f"Workflow '{workflow_name}' not found."}
    
    del workflows["workflows"][workflow_name]
    save_workflows(workflows)
    return {"status": "success", "message": f"✅ Workflow '{workflow_name}' removed successfully."}

def main():
    parser = argparse.ArgumentParser(description="Create Workflow Tool CLI")
    parser.add_argument("action", choices=["get_supported_actions", "search_workflow", "add_workflow", "remove_workflow"], help="Action to perform")
    ...
    elif args.action == "remove_workflow":
        if "workflow_name" not in params:
            result = {"status": "error", "message": "❌ 'workflow_name' is required for remove_workflow."}
        else:
            result = remove_workflow(params["workflow_name"])
    else:
